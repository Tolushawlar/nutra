...
function SearchForm() {
  ...
  const { searchResults, setSearchResults, displayResults, setDisplayResults, searchTerm, setSearchTerm } = context;

  const [dropdownVisible, setDropdownVisible] = useState(false);
  const [searchMode, setSearchMode] = useState('FOOD'); // 'FOOD', 'PRICE', or 'BOTH'
  const [input, setInput]: any = useState("")
  const [placeholder, setPlaceholder] = useState('Search for food e.g rice, beans, snacks');
  ...
  return (
    <div className='hover:shadow-lg  relative my-10  w-[85vw] md:w-[60vw] h-[30px] md:h-[80px] py-[40px] px-4  bg-white md:flex shadow-md flex-row items-center justify-between  rounded-[16px] md:rounded-[24px] border-[1px] border-[#a9c1a9]'>
      <div className='relative bottom-[2vh] md:bottom-0 flex flex-row justify-center items-center'>
        <Image
          src={search}
          alt="cartImage"
      ...
      <div className='hidden bg-[#F1F7F0] w-[500px] h-[60px] md:flex flex-row justify-center items-center rounded-[16px] shadow-sm'>
        ...
        <button
          ...
          onClick={() => handleButtonClick('PRICE', 'Click the price range to search ',)}
        // onBlur={() => setDropdownVisible(false)}
        ...
        <button
          ...
          onClick={() => handleButtonClick('BOTH', 'Search by both food and price.')}
      ...
      </div>
      {
        dropdownVisible && (
          <div className='z-[99999999999999] absolute top-[100px] left-0 w-[100%] bg-white shadow-lg rounded-[16px] max-h-[300px] overflow-auto'>
            ...
            {searchMode === 'PRICE' && (
              <div>
                <div className='flex flex-row justify-start gap-10 items-center p-4' onBlur={() => setDropdownVisible(true)}>
                  <div className='flex flex-row items-center justify-between w-[90vw]'>
                    {priceRanges.map((range, index) => (
                      <button
                        ...
                        onBlur={() => setDropdownVisible(true)}
                        className={`cursor-pointer font-semibold text-lg p-3 rounded-[8px] ${selectedPriceRange === range.value ? 'bg-[#BCF800] text-[#003d28]' : 'bg-[#003d28] text-[#BCF800]'}`}
                      >
                        {range.range}
                      </button>
                    ))}
                    <IoClose onClick={() => setDropdownVisible(false)} className="cursor-pointer w-[30px] h-[30px]" />
                  </div>
                </div>
                {searchResults.length === 0 ? (
                  <div className='flex items-center p-4 border-b border-gray-200' onBlur={() => setDropdownVisible(false)}>
                    <span className='text-gray-500'>No results found</span>
                  </div>
                ) : (
                  searchResults.map((item: any, index: any) => (
                    <div key={index} className='flex items-center p-4 border-b border-gray-200'>
                      <Image width={200} height={200} src={item.image} alt="image" className='w-[50px] h-[50px] object-cover rounded-full mr-4' />
                      <div className='flex flex-col items-start'>
            ...
            {searchMode === 'BOTH' && (
              <>
                ...
                {foodItems.map((item: any, index) => (
                  <div key={index} className='flex items-center p-4 border-b border-gray-200'>
                    <img src={item.image} alt={item.foodName} className='w-[50px] h-[50px] object-cover rounded-full mr-4' />
