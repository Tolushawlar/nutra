...
function SearchForm() {
  ...
  const [searchMode, setSearchMode] = useState("foodname");
    ...
    <div className="hidden relative my-10 w-[60vw] h-[80px] py-[40px] px-4 bg-white md:flex shadow-md flex-row items-center justify-between rounded-[24px] border-[1px] border-[#a9c1a9]">
      ...
      <div className="bg-[#F1F7F0] w-[500px] h-[60px] flex flex-row justify-center items-center rounded-[16px]">
        <button
          ...
          onClick={() => handleButtonClick("foodname")}
          onBlur={() => setDropdownVisible(false)}
        >
          SEARCH BY FOOD
        </button>
            ...
            <>
              <div className="flex flex-row justify-start gap-10 items-center p-4">
                {priceRanges.map((range, index) => (
                  <span
                    key={index}
                    className="font-semibold text-lg bg-[#003d28] p-3 rounded-[8px] text-[#BCF800]"
                  >
                    {range.range}
                  </span>
                ))}
